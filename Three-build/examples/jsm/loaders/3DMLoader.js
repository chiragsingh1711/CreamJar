...
class Rhino3dmLoader extends Loader {
  ...
  _createMaterial(material, renderEnvironment) {
    ...
    if (
      material.pbrSupported &&
      material.pbr.opacity === 0 &&
...
function Rhino3dmWorker() {
  ...
  function decodeObjects(rhino, buffer) {
    ...
    const reflectionId = doc.settings().renderSettings()
      .renderEnvironments.reflectionId;
    ...
    const renderSettings = {
      ...
      backgroundColorBottom: doc.settings().renderSettings()
      ...
      scaleBackgroundToFit: doc.settings().renderSettings()
        .scaleBackgroundToFit,
      transparentBackground: doc.settings().renderSettings()
      ...
      groundPlane: extractProperties(
        doc.settings().renderSettings().groundPlane
      ),
      ...
      linearWorkflow: extractProperties(
        doc.settings().renderSettings().linearWorkflow
      ),
      renderChannels: extractProperties(
        doc.settings().renderSettings().renderChannels
      ),
      sun: extractProperties(doc.settings().renderSettings().sun),
      renderEnvironments: extractProperties(
        doc.settings().renderSettings().renderEnvironments
      ),
      postEffects: extractProperties(
        doc.settings().renderSettings().postEffects
      ),
    };
